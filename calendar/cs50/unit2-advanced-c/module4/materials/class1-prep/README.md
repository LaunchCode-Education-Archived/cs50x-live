...
Lecture Notes | [Week 4 / Pointers](http://cdn.cs50.net/2015/fall/lectures/4/m/notes4m/notes4m.html#pointers)| Read | Gist: The value of a `string` variable is not actually the contents of the text: instead it is a number representing the memory address where the text is actually lcoated-- more specifically, where the first `char` is located. In other words, a `string` is a `pointer` to a `char`. 
